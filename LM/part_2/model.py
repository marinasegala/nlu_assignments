...
class VarDropout(nn.Module):
    ...
    def forward(self, x):
        ...
        m = bernoulli_distribution.sample(size_batch, 1, size_emb).to(
            DEVICE)  # x.new (size_batch, 1, size_emb).bernoulli_(1 - self.drop) #for each element in the batch, we will drop the same words
        mask = m / (1 - self.drop)
